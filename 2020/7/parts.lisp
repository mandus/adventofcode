...
(defun content-list (lst &optional (found nil))
  (let* ((bag (car (cl-ppcre:split " bag" (string-trim " " (car lst)))))
         ...
         (bag-count (string-trim " " (subseq bag 0 first-space)))
         (bag-type (string-trim " " (subseq bag first-space)))
         (nxtfound (cons bag-type (cons bag-count found)))
         (more (cdr lst))
         ) 
    (cond
...
(defun hashdata (data) 
  ...
  (dolist (line data) 
    (let* ((bag (cl-ppcre:split "contain " line))
           (name (string-trim " " (car (cl-ppcre:split "bags" (car bag)))))
           (content (content-list (cl-ppcre:split "," (string-trim "." (cadr bag)))))) 
      (when *debug*
